      ...
      <el-tab-pane name="first">
        ...
        <section class="append">
          <ul>
            ...
            <li v-if="optionValue == '选择题' || optionValue == '填空题' || optionValue == '判断题' || optionValue == '主观题'">
              <span>所属章节：</span>
              <el-input
                placeholder="请输入对应章节"
                v-model="getPostModel().section"
                class="w150"
                clearable
              >
              </el-input>
          ...
          <!-- 主观题 -->
          <div class="change subjective" v-if="optionValue == '主观题'">
            <div class="title">
              <el-tag>题目:</el-tag><span>在下面的输入框中输入主观题题目</span>
              <el-input
                type="textarea"
                rows="4"
                v-model="postSubjective.question"
                placeholder="请输入题目内容"
                resize="none"
                class="answer"
              >
            ...
            <div class="fillAnswer">
              ...
              <el-input
                type="textarea"
                rows="4"
                v-model="postSubjective.referenceAnswer"
                placeholder="请输入参考答案"
                resize="none"
                class="answer"
              >
            ...
            <div class="title analysis">
              ...
              <el-input
                type="textarea"
                rows="4"
                v-model="postSubjective.analysis"
                placeholder="请输入答案解析"
                resize="none"
                class="answer"
              >
            ...
            <div class="submit">
              <el-button type="primary" @click="subjectiveSubmit()"
                >立即添加</el-button
              >
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    subjectiveSubmit() {
      //主观题提交
      this.postSubjective.subject = this.subject;
      this.$axios({
        url: "/api/subjectiveQuestion",
        method: "post",
        data: {
          ...this.postSubjective,
        },
        ...
        .then(() => {
          // 获取最新添加的主观题ID (假设后端提供了对应的接口，或者我们通用处理)
          // 实际上主观题可能还没写这个接口，我先按照规律写，或者直接由后端在add时返回ID更好
          // 这里的业务逻辑是添加后立即关联到试卷
          this.$axios(`/api/subjectiveQuestionId`).then((res) => {
            let questionId = res.data.data.questionId;
            this.postPaper.questionId = questionId;
    ...
    getPostModel() {
        switch(this.optionValue) {
            case '选择题': return this.postChange;
            case '填空题': return this.postFill;
            case '判断题': return this.postJudge;
            case '主观题': return this.postSubjective;
            default: return {};
        }
    }
  },
